...
type PostData = { slug: string; title: string; summary: string | null; contentMd: string; coverImage: string | null; status: Status; isPinned: boolean; scheduledAt: Date | string | null; tags?: { tag: TagOption }[]; seriesPost?: { seriesId: string; sortOrder: number; series: SeriesOption }[] }
...
export function PostEditorForm({ mode, post, tags, series }: { mode: 'create' | 'edit'; post?: PostData; tags: TagOption[]; series: SeriesOption[] }) {
  ...
  async function save(targetStatus: Status) {
    ...
    const payload = { title: title.trim(), contentMd, summary: summary.trim() || undefined, coverImage: coverImage.trim() || null, status: targetStatus, isPinned, scheduledAt: targetStatus === 'scheduled' ? scheduledDate!.toISOString() : undefined, tagIds, seriesId: seriesId || undefined, seriesOrder }
    try {
      ...
      if (mode === 'create') router.replace(`/admin/posts/${data.slug}/edit`); router.refresh()
