...
import { PostRevisionPanel } from '@/components/console/PostRevisionPanel'
...
type PostData = { id?: string; slug: string; title: string; summary: string | null; contentMd: string; coverImage: string | null; status: Status; isPinned: boolean; visibility?: 'public' | 'private'; allowComment?: boolean; scheduledAt: Date | string | null; category?: { id: string; name: string; slug: string } | null; tags?: { tag: TagOption }[]; seriesPost?: { seriesId: string; sortOrder: number; series: SeriesOption }[] }
...
export function PostEditorForm({ mode, post, tags, series, categories }: { mode: 'create' | 'edit'; post?: PostData; tags: TagOption[]; series: SeriesOption[]; categories: CategoryOption[] }) {
  ...
  const [allowComment, setAllowComment] = useState(post?.allowComment ?? true)
  ...
  async function save(targetStatus: Status) {
    ...
    const payload = { title: title.trim(), contentMd, summary: summary.trim() || undefined, coverImage: coverImage.trim() || null, status: targetStatus, isPinned, visibility, allowComment, scheduledAt: targetStatus === 'scheduled' ? scheduledDate!.toISOString() : undefined, tagIds, categoryId: categoryId || null, seriesId: seriesId || undefined, seriesOrder }
    try {
  ...
  return <div className="p-8"><div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-bold text-text-primary">{mode === 'create' ? '新建文章' : '编辑文章'}</h1><p className="mt-2 text-text-secondary">支持 Markdown、预览、草稿、立即或定时发布。</p></div><Link href="/console/posts"><Button variant="secondary">返回列表</Button></Link></div>
    <div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]"><Card><CardHeader><h2 className="text-lg font-semibold text-text-primary">文章内容</h2></CardHeader><CardContent className="space-y-4">
    ...
    </CardContent></Card><div className="space-y-6"><Card><CardHeader><h2 className="font-semibold text-text-primary">发布设置</h2></CardHeader><CardContent className="space-y-4"><label className="block text-sm font-medium text-text-primary">目标状态<select value={status} onChange={(e) => { setStatus(e.target.value as Status); markDirty() }} className="mt-1.5 w-full rounded-button border border-border bg-background-base px-4 py-2"><option value="draft">草稿</option><option value="published">立即发布</option><option value="scheduled">定时发布</option></select></label>{status === 'scheduled' && <Input label="定时发布时间" type="datetime-local" value={scheduledAt} onChange={(e) => { setScheduledAt(e.target.value); markDirty() }} />}<Input label="封面图片 URL" value={coverImage} onChange={(e) => { setCoverImage(e.target.value); markDirty() }} placeholder="https://..." /><label className="block text-sm font-medium text-text-primary">可见性<select value={visibility} onChange={(e) => { setVisibility(e.target.value as 'public' | 'private'); markDirty() }} className="mt-1.5 w-full rounded-button border border-border bg-background-base px-4 py-2"><option value="public">公开 — 所有人可见</option><option value="private">私有 — 仅博主登录可见</option></select></label><label className="flex items-center gap-2 text-sm text-text-primary"><input type="checkbox" checked={allowComment} onChange={(e) => { setAllowComment(e.target.checked); markDirty() }} />允许评论</label><label className="flex items-center gap-2 text-sm text-text-primary"><input type="checkbox" checked={isPinned} onChange={(e) => { setIsPinned(e.target.checked); markDirty() }} />置顶文章</label><div className="flex flex-wrap gap-2"><Button type="button" loading={saving && status === 'draft'} onClick={() => save('draft')}>保存草稿</Button><Button type="button" variant="secondary" loading={saving && status !== 'draft'} onClick={() => save(status === 'draft' ? 'published' : status)}>{status === 'scheduled' ? '设置定时发布' : '发布/更新'}</Button></div></CardContent></Card>
      ...
      <Card><CardHeader><h2 className="font-semibold text-text-primary">Markdown 预览</h2></CardHeader><CardContent>{previewHtml ? <article className="prose prose-sm max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: previewHtml }} /> : <p className="text-sm text-text-muted">点击“刷新预览”检查最终渲染效果。</p>}</CardContent></Card>
      {mode === 'edit' && post?.id && <PostRevisionPanel postId={post.id} onRestored={() => router.refresh()} />}
    </div></div>
