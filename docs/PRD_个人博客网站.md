...
| 学习路线 | 路线节点管理、与文章关联、进度展示 | P0 |
| 项目展示 | 项目卡片、技术栈标签、源码/演示链接 | P1 |
| 时间线 | 职业与技术成长关键节点展示 | P1 |
| 评论系统 | 文章评论、嵌套回复、评论通知（v1.0 仅后台未读标记） | P1 |
| 互动 | 文章点赞、文章收藏 | P1 |
| 订阅 | RSS 订阅（v1.0 不实现，仅保留收藏功能） | - |
| 全站搜索 | 文章与动态全文检索 | P1 |
...
- **空状态（Empty State）：** 文章列表为空时展示「还没有文章，开始写第一篇吧」引导入口；搜索无结果时展示「未找到相关内容」+ 推荐热门文章；评论区为空时展示「暂无评论，来说点什么吧」；学习路线为空时展示「暂无学习路线，开始规划吧」
...
| site_favicon | varchar(500) | | 站点Favicon URL |
| avatar_url | varchar(500) | | 博主头像URL |
| bio | text | | 博主简介 |
...
#### 学习路线数据模型设计
- `learning_routes` 存储路线基本信息，通过 `is_public` 控制是否对外展示
- `learning_nodes` 支持多级结构（通过 `parent_id` 自引用），便于组织复杂的知识体系
- 每个节点可关联一篇文章（`post_id`），实现路线与内容的联动
- 节点状态（`status`）支持三种：学习中、已完成、计划中，用于展示学习进度
- 整体进度计算：`已完成节点数 / 总节点数 * 100%`

#### 个人资料模块化设计
- 采用三张独立表（`skills`、`social_links`、`work_experience`）存储个人资料
- 优势：结构清晰，便于独立扩展（如技能栈可添加熟练度、分类等字段）
- `skills` 支持分类和熟练度，便于按类别展示或生成技能雷达图
- `social_links` 通过 `is_visible` 控制展示哪些社交链接
- `work_experience` 使用 `end_date = NULL` 表示「至今」，前端展示时需特殊处理

#### 站点设置单行设计
- `site_settings` 表采用单行设计（仅一条记录），存储全局配置
