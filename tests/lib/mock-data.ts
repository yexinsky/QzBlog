...
export const xssTestCases = [
  { name: 'Script 标签', input: '<script>alert("XSS")</script>', expected: '' },
  { name: 'onerror 事件', input: '<img src=x onerror="alert(1)">', expected: '' },
  { name: 'onclick 事件', input: '<div onclick="alert(1)">点击</div>', expected: '<div>点击</div>' },
  { name: 'JavaScript 协议', input: '<a href="javascript:alert(1)">链接</a>', expected: '<a href="#">链接</a>' },
  { name: 'iframe', input: '<iframe src="https://evil.com"></iframe>', expected: '' },
  { name: '内联样式 JavaScript', input: '<div style="background: url(javascript:alert(1))">内容</div>', expected: '<div >内容</div>' },
  { name: '表情符号（安全）', input: '🎉 恭喜发财！', expected: '🎉 恭喜发财！' },
];

export const markdownTestCases = [
  { name: '基本标题', input: '# 标题\n## 二级标题\n### 三级标题', expected: ['h1', 'h2', 'h3'] },
  { name: '代码块', input: '```javascript\nconsole.log("Hello")\n```', expected: 'pre' },
  { name: '内联代码', input: '这是 `inline code` 示例', expected: 'code' },
  { name: '链接', input: '[链接文字](https://example.com)', expected: 'a' },
  { name: '列表', input: '- 项目1\n- 项目2\n- 项目3', expected: 'ul' },
  { name: '表格', input: '| 列1 | 列2 |\n|------|------|\n| 值1 | 值2 |', expected: 'table' },
  { name: 'LaTeX 数学公式', input: '行内公式 $E=mc^2$', expected: '$E=mc^2$' },
  { name: '任务列表', input: '- [x] 已完成\n- [ ] 未完成', expected: 'ul' },
];

export const searchTestCases = [
  { query: 'Go 并发', expectedResults: ['深入理解 Go 并发'], shouldInclude: ['Go', '并发'] },
  { query: 'react', expectedResults: ['React Hooks 最佳实践'], shouldInclude: ['React'] },
  { query: 'nonexistent', expectedResults: [], shouldInclude: [] },
];

export const rateLimitTestCases = [
  { endpoint: '/api/comments', limit: 3, windowMs: 60000, testIp: '192.168.1.100' },
  { endpoint: '/api/likes', limit: 10, windowMs: 60000, testIp: '192.168.1.100' },
  { endpoint: '/api/auth/login', limit: 5, windowMs: 60000, testIp: '192.168.1.100' },
];

export const paginationTestCases = [
  { page: 1, pageSize: 10, total: 25, expectedTotalPages: 3 },
  { page: 2, pageSize: 10, total: 25, expectedTotalPages: 3, expectedHasNext: true, expectedHasPrev: true },
  { page: 3, pageSize: 10, total: 25, expectedTotalPages: 3, expectedHasNext: false, expectedHasPrev: true },
];
