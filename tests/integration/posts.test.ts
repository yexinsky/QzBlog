...
let postsStore = mockPosts.map(p => ({ ...p, tags: [...p.tags] }));
...
const updatePost = async (id: string, data: UpdatePostData): Promise<typeof mockPosts[0] | null> => {
  ...
  postsStore[index] = { ...postsStore[index], ...data, updated_at: new Date().toISOString() };
...
const getPosts = async (options: {
  ...
  if (tag) filtered = filtered.filter((p) => p.tags.some((t) => t.slug === tag));
...
const publishPost = async (id: string): Promise<typeof mockPosts[0] | null> => {
  const index = postsStore.findIndex((p) => p.id === id);
  if (index === -1) return null;
  postsStore[index] = { ...postsStore[index], status: 'published', published_at: new Date().toISOString(), updated_at: new Date().toISOString() };
  return postsStore[index];
};

const schedulePost = async (id: string, scheduledAt: string): Promise<typeof mockPosts[0] | null> => {
  const index = postsStore.findIndex((p) => p.id === id);
  if (index === -1) return null;
  postsStore[index] = { ...postsStore[index], status: 'scheduled', scheduled_at: scheduledAt, cancel_scheduled: false, updated_at: new Date().toISOString() };
  return postsStore[index];
};
...
const cancelScheduledPost = async (id: string): Promise<typeof mockPosts[0] | null> => {
  const index = postsStore.findIndex((p) => p.id === id);
  if (index === -1) return null;
  postsStore[index] = { ...postsStore[index], status: 'draft', cancel_scheduled: true, updated_at: new Date().toISOString() };
  return postsStore[index];
};
...
describe('文章创建测试', () => {
  beforeEach(() => { postsStore = mockPosts.map(p => ({ ...p, tags: [...p.tags] })); });
  test('创建新文章', async () => {
    const data: CreatePostData = { title: '测试文章', slug: generateSlug('测试文章'), content_md: '# 测试文章\n\n这是测试内容。', status: 'draft' };
  ...
  test('创建文章时自动生成 slug', async () => {
    const data: CreatePostData = { title: 'Go Concurrency Programming', slug: generateSlug('Go Concurrency Programming'), content_md: '内容', status: 'published' };
    const post = await createPost(data);
    expect(post.slug).toBe('go-concurrency-programming');
  });
  test('创建定时发布文章', async () => {
    const scheduledAt = '2026-05-20T10:00:00Z';
    const data: CreatePostData = { title: '定时发布测试', slug: generateSlug('定时发布测试'), content_md: '定时发布内容', status: 'scheduled', scheduled_at: scheduledAt };
    const post = await createPost(data);
  ...
  test('创建时计算字数', async () => {
    ...
    const data: CreatePostData = { title: '字数统计测试', slug: generateSlug('字数统计测试'), content_md: content, status: 'draft' };
    const post = await createPost(data);
...
describe('文章读取测试', () => {
  ...
  test('按状态筛选文章', async () => {
    ...
    result.data.forEach((post) => { expect(post.status).toBe('published'); });
  });
  test('分页获取文章', async () => {
...
describe('文章删除测试', () => {
  ...
  test('删除文章后相关标签也应清理（外键约束）', async () => {
    ...
    if (post) { post.tags = []; await deletePost(id); }
...
describe('文章发布与定时发布测试', () => {
  ...
  test('取消定时发布', async () => {
    const id = '3c4d5e6f-7a8b-9c0d-1e2f-3g4h5i6j7k8';
  ...
  test('定时发布前 5 分钟内可取消', () => {
    ...
    scheduledAt.setMinutes(scheduledAt.getMinutes() + 3);
    const now = new Date();
    const canCancel = scheduledAt.getTime() - now.getTime() <= 5 * 60 * 1000 && scheduledAt.getTime() > now.getTime();
...
describe('文章列表与排序测试', () => {
  ...
  test('按发布时间降序排列', async () => {
    const result = await getPosts({ status: 'published' });
    const dates = result.data.filter((p) => p.published_at).map((p) => new Date(p.published_at!).getTime());
...
describe('文章搜索与筛选测试', () => {
  beforeEach(() => { postsStore = mockPosts.map(p => ({ ...p, tags: [...p.tags] })); });
  test('按标签筛选文章', async () => {
    const postsWithGoTag = postsStore.filter((p) => p.tags.some((t) => t.slug === 'go'));
    expect(postsWithGoTag.length).toBeGreaterThan(0);
    const result = await getPosts({ tag: 'go' });
    expect(result.data.length).toBeGreaterThan(0);
    result.data.forEach((post) => { expect(post.tags.some((t) => t.slug === 'go')).toBe(true); });
  });
  test('组合筛选条件', async () => {
    const result = await getPosts({ status: 'published', tag: 'go' });
